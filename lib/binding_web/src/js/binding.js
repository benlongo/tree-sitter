...
/* eslint-disable max-classes-per-file, no-underscore-dangle,
                  no-use-before-define, no-param-reassign */
/* eslint no-plusplus: ["error", { "allowForLoopAfterthoughts": true }] */
import C from '../../build-wasm/tree-sitter-c';
...
class Parser {
  ...
  parse(callback, oldTree, options) {
    if (typeof callback === 'string') {
      C.currentParseCallback = (index, _, endIndex) => callback.slice(index, endIndex);
...
class Language {
          ...
          switch (operator) {
            ...
            case 'eq?': {
              ...
              if (steps[2].type === 'capture') {
                ...
                predicates[i].push((captures) => {
                  const node1 = captures.find((c) => c.name === captureName1).node;
                  const node2 = captures.find((c) => c.name === captureName2).node;
                ...
                predicates[i].push((captures) => {
                  const capture = captures.find((c) => c.name === captureName);
                  if (capture) return (capture.node.text === stringValue) === isPositive;
                  return false;
                });
              }
              break;
            ...
            case 'match?': {
              ...
              predicates[i].push((captures) => {
                const capture = captures.find((c) => c.name === captureName);
                if (capture) return regex.test(capture.node.text);
                return false;
              });
              break;
            }
  ...
  static load(url) {
    ...
    return bytes
      .then((b) => C.loadWebAssemblyModule(b, { loadAsync: true }))
      .then((mod) => {
        const functionName = Object.keys(mod).find((key) => LANGUAGE_FUNCTION_REGEX.test(key)
...
function unmarshalRange(address) {
  ...
  result.startIndex = C.getValue(address, 'i32'); address += SIZE_OF_INT;
  result.endIndex = C.getValue(address, 'i32');
  return result;
}
